...
import (
	...
	"github.com/vibe-menu/internal/ui/arch"
	"github.com/vibe-menu/internal/ui/backend"
	"github.com/vibe-menu/internal/ui/contracts"
	"github.com/vibe-menu/internal/ui/core"
	"github.com/vibe-menu/internal/ui/crosscut"
	"github.com/vibe-menu/internal/ui/data"
	"github.com/vibe-menu/internal/ui/description"
...
type Model struct {
	...
	backendEditor     backend.BackendEditor
	dataTabEditor     data.DataTabEditor
	contractsEditor   contracts.ContractsEditor
	frontendEditor    frontend.FrontendEditor
	infraEditor       infra.InfraEditor
	crossCutEditor    crosscut.CrossCutEditor
	realizeEditor     realize_cfg.Editor
...
func NewModel(onSave SaveFunc) Model {
	ti := textinput.New()
	ti.Prompt = ""
	ti.TextStyle = lipgloss.NewStyle().Foreground(lipgloss.Color(core.ClrFg))
	ti.Cursor.Style = core.StyleCursor
	ti.PlaceholderStyle = lipgloss.NewStyle().Foreground(lipgloss.Color(core.ClrFgDim))
	...
	menu := provider.NewMenu()
	m := Model{
		sections:          core.InitSections(),
		textInput:         ti,
		textArea:          ta,
		descriptionEditor: description.NewEditor(),
		backendEditor:     backend.NewEditor(),
		dataTabEditor:     data.NewEditor(),
		contractsEditor:   contracts.NewEditor(),
		frontendEditor:    frontend.NewEditor(),
		infraEditor:       infra.NewEditor(),
		crossCutEditor:    crosscut.NewEditor(),
		realizeEditor:     realize_cfg.NewEditor(),
		realize:           realizeState{screen: realization.NewScreen()},
		arch:              archState{screen: arch.NewScreen()},
		modal:             modalState{menu: menu},
		onSave:            onSave,
	}
	// Wire the shared animation frame counter into the arch sub-package.
	arch.SetAnimFramePtr(&core.AnimFrame)
...
func (m Model) activeEditor() core.Editor {
...
func (m Model) activeMode() core.Mode {
...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	if _, ok := msg.(core.UITickMsg); ok {
		core.AnimFrame = (core.AnimFrame + 1) % 2
		return m, core.UITick()
	}

	if _, ok := msg.(realize_cfg.RealizeMsg); ok {
...
func (m Model) updateNormal(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	if m.modal.open {
		switch key.String() {
		case "M", "ctrl+c":
			if !m.modal.menu.IsCredentialFocused() {
				m.modal.open = false
				return m, nil
			}
		case "esc":
			...
			if m.modal.menu.IsProviderFocused() {
				m.modal.open = false
				return m, nil
			}
		}
	...
	case "tab", "L":
		if e := m.activeEditor(); e != nil && e.Mode() == core.ModeInsert {
			return m.delegateUpdate(msg)
		}
...
func (m Model) LoadManifestIntoModel(path string) (Model, error) {
	...
	if provPA, err := manifest.LoadProviders(manifest.ProvidersPath()); err == nil && len(provPA) > 0 {
		if m.modal.menu.Configured == nil {
			m.modal.menu.Configured = make(map[string]provider.ProviderSelection)
		}
		for label, pa := range provPA {
			m.modal.menu.Configured[label] = provider.ProviderSelection{
