...
// ── lookup maps ──────────────────────────────────────────────────────────────
// These are duplicated from backend_fields.go and frontend_fields_pages.go
// because the crosscut sub-package cannot import the parent ui package.
...
func computeTestingFields(backendLangs, backendProtocols []string, backendArchPattern, prevArchPattern, frontendLang, frontendFramework string, existing []core.Field) []core.Field {
	...
	fields := make([]core.Field, 0, len(template))
...
func defaultTestingFields() []core.Field {
...
func defaultStandardsFields() []core.Field {
	linterOpts := linterOptionsForLanguages(nil)
	return []core.Field{
		{
			Key: "dep_updates", Label: "Dep. Updates  ", Kind: core.KindSelect,
			Options: []string{"Dependabot", "Renovate", "Manual", "None"},
			Value:   "Dependabot",
		},
		{
			Key: "feature_flags", Label: "Feature Flags ", Kind: core.KindSelect,
...
func defaultDocsTailFields() []core.Field {
	return []core.Field{
		{
			Key: "auto_generate", Label: "auto_generate ", Kind: core.KindSelect,
			Options: core.OptionsOffOn, Value: "true", SelIdx: 1,
		},
		{
			Key: "changelog", Label: "changelog     ", Kind: core.KindSelect,
...
func (cc *CrossCutEditor) SetDocsContext(protocols []string) {
	if core.StringSlicesEqual(cc.docsProtocols, protocols) {
		return
	}
...
func (cc *CrossCutEditor) SetTestingContext(backendLangs, backendProtocols []string, backendArchPattern, frontendLang, frontendFramework string) {
	if core.StringSlicesEqual(cc.backendLangs, backendLangs) &&
		core.StringSlicesEqual(cc.backendProtocols, backendProtocols) &&
