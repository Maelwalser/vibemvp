...
type CrossCutEditor struct {
	...
	testingFields  []core.Field
	testFormIdx    int
	testingEnabled bool

	docsFields  []core.Field
	docsFormIdx int
	docsEnabled bool

	standardsFields  []core.Field
...
func (cc CrossCutEditor) ToManifestCrossCutPillar() manifest.CrossCutPillar {
	...
	if cc.testingEnabled {
		p.Testing = &manifest.TestingConfig{
			Unit:            core.NoneToEmpty(core.FieldGet(cc.testingFields, "unit")),
			Integration:     core.NoneToEmpty(core.FieldGet(cc.testingFields, "integration")),
			E2E:             core.NoneToEmpty(core.FieldGet(cc.testingFields, "e2e")),
			FrontendTesting: core.NoneToEmpty(core.FieldGet(cc.testingFields, "fe_testing")),
			API:             core.NoneToEmpty(core.FieldGet(cc.testingFields, "api")),
			Load:            core.NoneToEmpty(core.FieldGet(cc.testingFields, "load")),
			Contract:        core.NoneToEmpty(core.FieldGet(cc.testingFields, "contract")),
		}
	}
	if cc.docsEnabled {
		...
		for _, proto := range protos {
			...
			if v := core.FieldGet(cc.docsFields, key); v != "" && v != "None" {
				formats[proto] = v
			}
		}
		p.Docs = &manifest.DocsConfig{
			PerProtocolFormats: formats,
			AutoGenerate:       core.FieldGet(cc.docsFields, "auto_generate") == "true",
			Changelog:          core.NoneToEmpty(core.FieldGet(cc.docsFields, "changelog")),
		}
	}
	if cc.standardsEnabled {
		p.DependencyUpdates = core.FieldGet(cc.standardsFields, "dep_updates")
		p.FeatureFlags = core.FieldGet(cc.standardsFields, "feature_flags")
		p.UptimeSLO = core.FieldGet(cc.standardsFields, "uptime_slo")
		p.LatencyP99 = core.FieldGet(cc.standardsFields, "latency_p99")
		p.BackendLinter = core.FieldGet(cc.standardsFields, "be_linter")
		p.FrontendLinter = core.FieldGet(cc.standardsFields, "fe_linter")
	}
	return p
}
...
func (cc CrossCutEditor) FromCrossCutPillar(p manifest.CrossCutPillar) CrossCutEditor {
	if t := p.Testing; t != nil && (t.Unit != "" || t.Integration != "" || t.E2E != "") {
		cc.testingEnabled = true
		cc.testingFields = core.SetFieldValue(cc.testingFields, "unit", t.Unit)
		cc.testingFields = core.SetFieldValue(cc.testingFields, "integration", t.Integration)
		cc.testingFields = core.SetFieldValue(cc.testingFields, "e2e", t.E2E)
		cc.testingFields = core.SetFieldValue(cc.testingFields, "fe_testing", t.FrontendTesting)
		cc.testingFields = core.SetFieldValue(cc.testingFields, "api", t.API)
		cc.testingFields = core.SetFieldValue(cc.testingFields, "load", t.Load)
		cc.testingFields = core.SetFieldValue(cc.testingFields, "contract", t.Contract)
	}
	...
	if d != nil && (len(d.PerProtocolFormats) > 0 || d.APIDocs != "" || d.Changelog != "") {
		...
		if len(d.PerProtocolFormats) > 0 {
			...
			for proto, format := range d.PerProtocolFormats {
				cc.docsFields = core.SetFieldValue(cc.docsFields, docsFormatFieldKey(proto), format)
			}
		} else if d.APIDocs != "" {
			// Migrate legacy single-format field to REST.
			cc.docsFields = core.SetFieldValue(cc.docsFields, docsFormatFieldKey("REST"), d.APIDocs)
		}
		cc.docsFields = core.SetFieldValue(cc.docsFields, "auto_generate", boolStr(d.AutoGenerate))
		if d.Changelog != "" {
			cc.docsFields = core.SetFieldValue(cc.docsFields, "changelog", d.Changelog)
		}
	}

	if p.DependencyUpdates != "" || p.BackendLinter != "" || p.FrontendLinter != "" {
		cc.standardsEnabled = true
		cc.standardsFields = core.SetFieldValue(cc.standardsFields, "dep_updates", p.DependencyUpdates)
		cc.standardsFields = core.SetFieldValue(cc.standardsFields, "feature_flags", p.FeatureFlags)
		cc.standardsFields = core.SetFieldValue(cc.standardsFields, "uptime_slo", p.UptimeSLO)
		cc.standardsFields = core.SetFieldValue(cc.standardsFields, "latency_p99", p.LatencyP99)
		cc.standardsFields = core.SetFieldValue(cc.standardsFields, "be_linter", p.BackendLinter)
		cc.standardsFields = core.SetFieldValue(cc.standardsFields, "fe_linter", p.FrontendLinter)
	}
...
func (cc CrossCutEditor) Mode() core.Mode {
	if cc.internalMode == core.ModeInsert {
...
func (cc CrossCutEditor) HintLine() string {
	...
	return core.HintBar("j/k", "navigate", "gg/G", "top/bottom", "[n]j/k", "jump n lines", "Space/Enter", "cycle", "H", "cycle back", "D", "delete config", "a/i", "edit text", "h/l", "sub-tab")
}

// activeCCFieldPtr returns a pointer to the currently focused field.
func (cc *CrossCutEditor) activeCCFieldPtr() *core.Field {
	switch cc.activeTab {
	case ccTabTesting:
...
func (cc CrossCutEditor) updateCCDropdown(key tea.KeyMsg) (CrossCutEditor, tea.Cmd) {
	...
	cc.dd.OptIdx = core.NavigateDropdown(key.String(), cc.dd.OptIdx, len(f.Options))
	switch key.String() {
	case " ", "enter":
...
func (cc CrossCutEditor) Update(msg tea.Msg) (CrossCutEditor, tea.Cmd) {
	...
	switch key.String() {
	case "h", "left", "l", "right":
		cc.activeTab = ccTabIdx(core.NavigateTab(key.String(), int(cc.activeTab), len(ccTabLabels)))
		return cc, nil
	}
...
func (cc CrossCutEditor) updateInsert(msg tea.Msg) (CrossCutEditor, tea.Cmd) {
	...
	if ok {
		...
		case "esc":
			cc.saveInput()
			cc.internalMode = core.ModeNormal
			cc.formInput.Blur()
			return cc, nil
		case "tab":
			cc.saveInput()
...
func (cc CrossCutEditor) tryEnterInsert() (CrossCutEditor, tea.Cmd) {
	...
	for range n {
		...
		if f.CanEditAsText() {
			cc.internalMode = core.ModeInsert
...
func (cc CrossCutEditor) View(w, h int) string {
	...
	var lines []string
	lines = append(lines,
		core.StyleSectionDesc.Render("  # Cross-Cutting Concerns — testing strategy and documentation"),
		"",
		core.RenderSubTabBar(ccTabLabels, int(cc.activeTab), w),
		"",
	)
	...
	case ccTabTesting:
		if cc.testingEnabled {
			fl := core.RenderFormFields(w, cc.testingFields, cc.testFormIdx, cc.internalMode == core.ModeInsert, cc.formInput, cc.dd.Open, cc.dd.OptIdx)
			lines = append(lines, core.AppendViewport(fl, 0, cc.testFormIdx, h-ccHeaderH)...)
		} else {
			lines = append(lines, core.StyleSectionDesc.Render("  (not configured — press 'a' to configure)"))
		}
	case ccTabDocs:
		if cc.docsEnabled {
			fl := core.RenderFormFields(w, cc.docsFields, cc.docsFormIdx, cc.internalMode == core.ModeInsert, cc.formInput, cc.dd.Open, cc.dd.OptIdx)
			lines = append(lines, core.AppendViewport(fl, 0, cc.docsFormIdx, h-ccHeaderH)...)
		} else {
			lines = append(lines, core.StyleSectionDesc.Render("  (not configured — press 'a' to configure)"))
		}
	case ccTabStandards:
		if cc.standardsEnabled {
			fl := core.RenderFormFields(w, cc.standardsFields, cc.standardsFormIdx, cc.internalMode == core.ModeInsert, cc.formInput, cc.dd.Open, cc.dd.OptIdx)
			lines = append(lines, core.AppendViewport(fl, 0, cc.standardsFormIdx, h-ccHeaderH)...)
		} else {
			lines = append(lines, core.StyleSectionDesc.Render("  (not configured — press 'a' to configure)"))
		}
	}
...
func (cc *CrossCutEditor) CurrentField() *core.Field {
	switch cc.activeTab {
	case ccTabTesting:
