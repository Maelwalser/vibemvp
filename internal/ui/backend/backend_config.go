...
func (be BackendEditor) updateStackConfigList(key tea.KeyMsg) (BackendEditor, tea.Cmd) {
	...
	case "a":
		be.stacksUndo.Push(core.CopyFieldItems(ed.items))
		newFields := defaultStackConfigFields()
		existing := make([]string, 0, len(ed.items))
		for _, item := range ed.items {
			if name := core.FieldGet(item, "name"); name != "" {
				existing = append(existing, name)
			}
		}
		newFields = core.SetFieldValue(newFields, "name", core.UniqueName("stack", existing))
...
func (be BackendEditor) viewStackConfigEditor(w int) []string {
	...
	if ed.itemView == beListViewList {
		var lines []string
		lines = append(lines, core.StyleSectionDesc.Render("  # Stack Configs — a: add  d: delete  Enter: edit"), "")
		if len(ed.items) == 0 {
			lines = append(lines, core.StyleSectionDesc.Render("  (no configs yet — press 'a' to add)"))
		} else {
			for i, item := range ed.items {
				...
				lang := core.FieldGet(item, "language")
				fw := core.FieldGet(item, "framework")
				extra := lang
				if fw != "" {
					extra += " / " + fw
				}
	...
	if n := core.FieldGet(ed.form, "name"); n != "" {
		name = n
	}
