...
import (
	...
	infraPkg "github.com/vibe-menu/internal/ui/infra"
)
...
func (be BackendEditor) updateServiceList(key tea.KeyMsg) (BackendEditor, tea.Cmd) {
	...
	case "a":
		be.svcsUndo.Push(core.SvcSnapshot{Items: core.CopyFieldItems(ed.items), Services: core.CopySlice(be.Services)})
		...
		if be.currentArch() == "monolith" {
			newFields = core.WithoutField(newFields, "health_deps")
		}
		...
		ed.form = core.SetFieldValue(ed.form, "name", core.UniqueName("service", existing))
		ed.formIdx = 0
...
func (be *BackendEditor) updateServiceFrameworkOptions(ed *beListEditor) {
	lang := core.FieldGet(ed.form, "language")
	opts, ok := backendFrameworksByLang[lang]
	if !ok {
		opts = []string{"Other"}
	}
	langVers, hasVers := core.LangVersions[lang]
	for i := range ed.form {
...
func (be *BackendEditor) updateServiceVersionOptions(ed *beListEditor) {
	lang := core.FieldGet(ed.form, "language")
	langVer := core.FieldGet(ed.form, "language_version")
	fw := core.FieldGet(ed.form, "framework")
...
// value if valid.
func (be *BackendEditor) applyServiceDiscoveryOpts(fields []core.Field) {
...
func (be *BackendEditor) updateEnvOrchestratorOptions() {
	computeEnv := core.FieldGet(be.EnvFields, "compute_env")
	opts, ok := infraPkg.OrchestratorByComputeEnv[computeEnv]
	if !ok {
...
func (be *BackendEditor) updateEnvMonolithVersionOptions() {
	lang := core.FieldGet(be.EnvFields, "monolith_lang")
	langVer := core.FieldGet(be.EnvFields, "monolith_lang_ver")
...
func (be BackendEditor) updateCommList(key tea.KeyMsg) (BackendEditor, tea.Cmd) {
	...
	case "a":
		be.commsUndo.Push(core.CommSnapshot{Items: core.CopyFieldItems(ed.items), Comms: core.CopySlice(be.CommLinks)})
	...
	case "enter":
		if n > 0 {
			ed.form = be.withDTONames(be.withServiceNames(core.CopyFields(ed.items[ed.itemIdx])))
			ed.formIdx = 0
			ed.itemView = beListViewForm
			be.activeField = 0
		}
...
func (be BackendEditor) isCommFieldHidden(key string) bool {
	if key == "response_dto" {
		direction := core.FieldGet(be.commEditor.form, "direction")
...
func (be BackendEditor) updateMessaging(key tea.KeyMsg) (BackendEditor, tea.Cmd) {
	...
	case "enter", " ":
		if be.activeField < brokerCount {
			...
			if eventIdx < eventCount {
				ed.form = be.withEventNames(core.CopyFields(ed.items[eventIdx]))
				ed.formIdx = 0
				ed.itemIdx = eventIdx
				ed.itemView = beListViewForm
			}
	...
	case "a":
		be.eventsUndo.Push(core.EventSnapshot{Items: core.CopyFieldItems(ed.items), Events: core.CopySlice(be.Events)})
		...
		ed.form = core.SetFieldValue(ed.form, "name", core.UniqueName("event", existing))
		ed.formIdx = 0
		ed.itemView = beListViewForm
...
func (be *BackendEditor) saveEventForm() {
	...
	evt := manifest.EventDef{
		...
		PublisherService: core.FieldGet(ed.form, "publisher_service"),
		ConsumerService:  core.FieldGet(ed.form, "consumer_service"),
		DTO:              core.FieldGet(ed.form, "dto"),
		Description:      core.FieldGet(ed.form, "description"),
	}
